...
def targets_probs(predicted_probs, targets):
    ...
    """
    probs_targets = predicted_probs[np.arange(len(predicted_probs)), targets]
...
def get_n_correct(output, targets):
    ...
    predicted_idxs = np.argmax(predicted_probs, axis=1)
...
def target_in_topk(output, targets, k):
    ...
    topk = torch.topk(torch.from_numpy(predicted_probs), 10)
...
def get_perword_metrics_sentence(output, targets, sentence, tags):
    """
    ...
    metrics_dict = {"actual_word": list(sentence)[:-1], \
                 "actual_pos": tags[:-1], \
                 "next_word": list(sentence)[1:], \
                 "next_tag": tags[1:], \
                 "correct": correct, \
                 "entropy": entropy, \
